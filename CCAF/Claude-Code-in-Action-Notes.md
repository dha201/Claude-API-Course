...
> Personal study reference compiled from Anthropic's **"Claude Code in Action"** course
> (anthropic.skilljar.com). Captured 2026-06-28. Contains the structured notes for the 13
> video teaching lessons, grouped into 5 themes.
>
> **Not captured here** (these lessons load their body client-side / are hands-on, so their full
> text isn't in the page source): *Introduction*, *Claude Code setup*, *Project setup*,
> *Gotchas around hooks*, *Another useful hook*, *Summary and next steps*. Their concepts are
> largely reflected in the notes below. A few hook/SDK lessons include a second "alternate notes"
> take from the source.
>
> *Note: examples reference Claude Code as taught in the course; verify current CLI flags,
> settings paths (`.claude/`), and SDK options against the live docs before relying on them.*

## Contents

1. Foundations — What is a Coding Assistant?, Claude Code in Action
2. Core Workflow — Adding Context, Making Changes, Controlling Context, Custom Commands
3. Extending Claude Code — MCP Servers, GitHub Integration
4. Hooks — Introducing, Defining, Implementing, Useful Hooks
5. Automation — The Claude Code SDK

---


## 1 &middot; Foundations
...
## 2 &middot; Core Workflow — Context & Changes
...
## 3 &middot; Extending Claude Code
...
### Defining Hooks (alternate notes)
...
### Implementing a Hook (alternate notes)
...
## 5 &middot; Automation — The Claude Code SDK
...
### The Claude Code SDK (alternate notes)
...
> Compiled from the *Claude Code in Action* course quiz. The **correct** answer is marked âœ“,
> followed by a one-line rationale. Grouped by theme.

## Foundations

**Q. What is the fundamental limitation of language models that necessitates the use of a tool system in coding assistants?**
...
- âœ“ **They can only process text input/output and cannot directly interact with external systems**

*Why:* an LLM is text-in/text-out; tools bridge it to files, commands, and external systems so it can act.

## Core Workflow â€” Context & Changes

**Q. Which of the following correctly describes the three types of Claude.md files and their usage?**
...
- âœ“ **Project level (shared with team, committed), Local level (personal, not committed), Machine level (global for all projects)**
- Project level (configuration), Local level (documentation), Machine level (automation)

*Why:* project = committed/team, local = personal/uncommitted, machine = global across all your projects.

**Q. What is the primary difference between Plan Mode and Thinking Mode in Claude Code?**
...
- âœ“ **Plan Mode handles breadth (multi-step tasks) while Thinking Mode handles depth (complex logic)**

*Why:* Plan researches widely across files; Thinking gives deeper reasoning budget for hard logic. Both cost extra tokens.

**Q. How do you create a custom command in Claude Code that accepts runtime parameters?**
...
- âœ“ **Include `$ARGUMENTS` placeholder in the markdown command file**

*Why:* commands are markdown files in `.claude/commands/`; `$ARGUMENTS` injects the runtime argument string.

## Extending Claude Code

**Q. What permission configuration is required when integrating MCP servers with Claude Code in GitHub Actions?**
- âœ“ **Each MCP server tool must be individually listed in the permissions**
...
*Why:* in Actions you must explicitly list every permission â€” MCP tools get no shortcuts/wildcards.

## Hooks

**Q. Which type of hook can prevent a tool call from happening if certain conditions are met?**
- PostToolUse hook
- Project hook
- Global hook
- âœ“ **PreToolUse hook**

*Why:* PreToolUse runs before the tool and can block it (exit code 2); PostToolUse runs after and can't block. "Project/Global" describe scope, not timing.

**Q. A developer wants to prevent Claude from reading sensitive `.env` files. Which type of hook should they set up, and what tool names would they likely match?**
- PostToolUse hook, matching Write and Edit
- PreToolUse hook, matching Write and Create
- âœ“ **PreToolUse hook, matching Read and Grep**
- PostToolUse hook, matching Read and Delete

*Why:* to *block* a read you need a PreToolUse hook, and the tools that read file contents are Read and Grep (matcher `read|grep`).
