...
> Personal study reference compiled from Anthropic's **"Building with the Claude API"** course
> (anthropic.skilljar.com). Contains the structured notes for all 75 teaching lessons across
> 10 modules, plus the "Fine Grained Tool Calling" transcript. Captured 2026-06-28.
>
> *Note: code/model examples reflect the course as recorded (it references `claude-3-sonnet`).
> Verify current Claude model IDs and pricing before reusing examples in production.*
...
## 2 &middot; Accessing Claude with the API
...
## 4 &middot; Prompt Engineering Techniques
...
## 6 &middot; RAG and Agentic Search
...
## 8 &middot; Model Context Protocol
...
## 9 &middot; Anthropic Apps & Claude Code
...
## 10 &middot; Agents and Workflows
...
> Compiled from the course quizzes and the Final Assessment for *Building with the Claude API*.
> The **correct** answer is marked âœ“, followed by a one-line rationale. Grouped by module.

## Accessing Claude with the API

**Q. You want to send a message to Claude through the API. Which four things do you absolutely need to include?**
- Temperature, creativity, speed, and language
- Date, time, location, and device type
- âœ“ **API key, model name, messages, and max tokens**
- Username, password, email, and phone number

*Why:* these four are the required arguments of `client.messages.create()`; the rest are optional or irrelevant.

**Q. You're building a web app that talks to Claude. Where should you store your API key?**
- âœ“ **On your server, hidden from users**
- In a public GitHub repository
- In the web browser's settings
- In your website's JavaScript code

*Why:* never expose the key client-side; the browser/JS/public repo would leak it. Calls go through your server.

**Q. You ask Claude "What is pizza?" and it answers. Then you ask "What toppings are popular?" but Claude doesn't know you're still talking about pizza. What's the problem?**
- âœ“ **You need to send the whole conversation history with each request**
- Claude doesn't like pizza
- Claude is broken
- You're asking too many questions

*Why:* the API is stateless â€” it stores no memory, so you must resend the full message list every turn.

**Q. You want Claude to write very creative, unpredictable stories. What temperature setting should you use?**
- 0.5 (medium)
- 0.0 (very low)
- Temperature doesn't affect creativity
- âœ“ **1.0 (very high)**

*Why:* higher temperature increases randomness/variety; low temperature is deterministic and repetitive.

**Q. You're making a math tutoring app. You want Claude to give hints instead of direct answers. What should you use?**
- Ask users to say "please" in every question
- Use smaller fonts to make Claude quiet
- Send messages only on weekdays
- âœ“ **A system prompt explaining Claude should act like a tutor**

*Why:* the system prompt sets Claude's role and behavioral rules for the whole conversation.

## Prompt Engineering

**Q. You want an AI to write a product description. Which opening is most clear and direct?**
...
- âœ“ **"Write a product description for running shoes."**
- "What do you think makes a good product description?"

*Why:* a direct imperative with a concrete subject beats vague, hedged, or open-ended phrasing.

**Q. You're asking an AI to analyze both customer reviews and sales data. How should you organize this information in your prompt?**
...
- âœ“ **Use XML tags like `<reviews>` and `<sales_data>` to separate them**

*Why:* XML tags give clear structure so Claude can tell the two data sources apart.

## Prompt Evaluation

**Q. You're improving a prompt that isn't working well. What should you do after applying a prompt engineering technique?**
- Start over with a completely new prompt
- Ask someone else to write the prompt
- âœ“ **Use prompt evaluations to see if it actually improved**
- Move on to the next technique immediately

*Why:* you must measure whether the change helped; iterate â†’ evaluate â†’ repeat.

**Q. You want to measure how well your AI prompt actually works in practice. Which approach should you focus on?**
- Using multishot prompting examples
- Prompt engineering techniques like XML tags
- Writing longer, more detailed prompts
- âœ“ **Prompt evaluation with automated testing**

*Why:* those others improve a prompt; only evaluation *measures* how well it works.

**Q. What is a model grader in prompt evaluation?**
- A human reviewer who manually scores AI outputs
- âœ“ **Another AI model used to assess the quality of outputs**
- A programmatic check that validates syntax and format
- A scoring system that only measures response speed

*Why:* model-based grading uses an LLM to judge quality (vs. code-based grading, which checks syntax/format).

**Q. You're running a prompt evaluation. After getting responses from Claude, what's the next step in the typical workflow?**
...
- âœ“ **Feed the responses through a grader for scoring**

*Why:* the workflow is generate dataset â†’ run prompt â†’ grade the responses.

## Tool Use with Claude

**Q. What is a tool function in the context of Claude's tool use system?**
...
- âœ“ **A plain function that gets executed when Claude needs additional information or needs to perform an action**

*Why:* it's ordinary code in your app; a schema tells Claude when/how to call it.

**Q. What is the primary purpose of tool use in Claude?**
- To automatically save conversation history
- âœ“ **To allow Claude to access real-time information and external systems beyond its training data**
- To make Claude respond faster to user queries
- To reduce the number of tokens Claude uses in responses

*Why:* tools let a text-only model fetch data and take actions in the outside world.

**Q. What is the primary purpose of a batch tool in Claude's tool system?**
- âœ“ **To accept multiple tool calls and execute them simultaneously**
...
*Why:* the batch tool bundles several tool calls so they run in parallel.

**Q. Claude responds to your request with both explanatory text and a tool use block. What type of message structure is this?**
- An error message that needs to be fixed
- âœ“ **A multi-block message with different content types**
- A single-block text message
- A malformed response that should be ignored

*Why:* a single assistant message can contain multiple content blocks (e.g., text + tool_use).

## Features of Claude

**Q. You're building an app where users need to verify information Claude provides from documents. What feature should you enable?**
- Code execution
- Prompt caching
- âœ“ **Citations**
- Extended thinking

*Why:* Citations tie Claude's claims back to the exact source passages so users can verify them.

**Q. You keep sending the same long document to Claude with different questions. How can you make this faster and cheaper?**
- Compress the document first
- Split the document into smaller pieces
- âœ“ **Use prompt caching with cache breakpoints**
- Ask multiple questions at once

*Why:* caching the repeated document avoids re-processing it each request, cutting latency and cost.

## Model Context Protocol

**Q. You're deciding how to implement a new feature in your MCP server. Users should be able to click a button to trigger a "summarize document" workflow. Which MCP primitive should you use?**
- Resources â€” because you need to fetch document data
- Functions â€” because it involves processing
- âœ“ **Prompts â€” because users control when to start the workflow**
- Tools â€” because the AI needs new capabilities

*Why:* Prompts are user-controlled (surfaced as buttons/commands); Resources are app-controlled data, Tools are model-controlled, "Functions" isn't an MCP primitive.

**Q. What is the Model Context Protocol (MCP)?**
- A security protocol for protecting AI model parameters
- âœ“ **A communication layer that provides Claude with context and tools without requiring tedious integration code**
- A programming language designed specifically for AI applications
- A database system for storing AI training data

*Why:* MCP is a standard integration layer for exposing tools/resources/prompts to models.

**Q. What is the primary difference between an MCP Server and an MCP Client in terms of their roles?**
- âœ“ **MCP Servers contain tools, prompts, and resources while MCP Clients act as the communication bridge to access those tools**
...
*Why:* servers expose capabilities; clients connect the model to those capabilities.

**Q. What does "transport agnostic" mean in the context of MCP communication?**
- âœ“ **MCP clients and servers can communicate using different methods like HTTP or standard input/output**
...
*Why:* the protocol works over multiple transports (stdio, HTTP) â€” it isn't tied to one.

## Anthropic Apps / Computer Use

**Q. Which of the following best describes Computer Use in the context of Claude?**
...
- âœ“ **A capability that lets Claude interact directly with desktop environments like a human would**
- A feature that optimizes Claude's processing speed on different hardware

*Why:* Computer Use lets Claude control a desktop (see the screen, move the mouse, type) like a person.

## Agents and Workflows

**Q. When should you choose workflows over agents for handling user tasks?**
...
- âœ“ **When you can picture the exact flow or steps Claude should go through to solve a problem**

*Why:* workflows suit known, fixed step sequences (reliable); agents suit open-ended, unpredictable tasks.

**Q. Which of the following best describes why environment inspection is crucial for AI agents?**
- It helps agents run faster and more efficiently
- âœ“ **It allows agents to observe and understand the results of their actions**
- It simplifies the agent's tool requirements
- It reduces the cost of running AI operations

*Why:* agents need feedback on how each action changed the environment to decide the next step.
